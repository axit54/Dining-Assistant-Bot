...
def validate_values(slots, name, value):
    ...
    # elif name == "Time":
    #     # if datetime.datetime.strptime("%s %s" % (slots['Date'], value), '%Y-%m-%d %H:%M') < datetime.datetime.now():
    #     if value<datetime.datetime.now()
    #         response = {"status": bad_stat, "message": "Oops, please provide a time in the future"}
    elif name == "Email":
        if '@' not in value:  
